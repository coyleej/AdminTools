...
\item Each node needs to copy the state files from the other system \emph{before} starting slurm. I could make this into a daemon that is required before \texttt{slurmctld} can start, but I'm going with the following for now. 
	...
	\item On the backup controller, stop \texttt{slurmctld} for testing purposes.

	\item Source \texttt{\textasciitilde /.bashrc}.	
...
\item To ensure seamless takeover, we will force the state transfer immediately prior to transferring control.

Open the admin's \texttt{.bash\_aliases} on both controllers and alias a command that transfers the slurm state immediately prior to issuing the takeover command.
